...
MyVector::MyVector(int const num) {
	data = nullptr; //  pointing to the array
	size_used = 0; // how many values that are in the array
	arr_capacity = num;
	data = new int[arr_capacity];
...
void MyVector::push_back(const int n) {
	if (arr_capacity == 0) {
		arr_capacity = 1;
	}
...
void MyVector::deep_copy(const MyVector& rho)
{
	size_used = rho.size();
...
ostream& operator<<(ostream& display, const MyVector& v) 
{
...
const int MyVector::at(const int n) const 
{
	if (n < 0 || n >= size_used)
	{
		throw n;
